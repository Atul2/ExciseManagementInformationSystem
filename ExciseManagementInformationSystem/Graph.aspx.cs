...
namespace ExciseManagementInformationSystem
{
     public partial class Graph : System.Web.UI.Page
    {
        ...
        #endregion
        #region GraphWareHouseWiseDispatch
        [WebMethod]
        public static List<GraphData> GraphWDisp(string selectedText1, string selectedText2, string selectedText3, string frmdt, string todt)
        {
            DispatchDataOperation ddo = new DispatchDataOperation();
            DataTable dt = new DataTable();
            dt = ddo.GraphWareHouseDispatch(selectedText1, selectedText2, selectedText3, frmdt, todt);
            List<GraphData> chartData = new List<GraphData>();
            foreach (DataRow dr in dt.Rows)
            {
                chartData.Add(new GraphData
                {
                    w = Convert.ToString(dr["WareHouseName"]).Remove(10),
                    x = (Convert.ToInt32(dr["No_Of_Dispatch"]))
                });
            }
            return chartData;
        }
        #endregion
        #region GraphWareHouseWiseInvoice
        [WebMethod]
        public static List<GraphData> GraphWInvo(string selectedText1, string selectedText2, string selectedText3, string frmdt, string todt)
        {
            DispatchDataOperation ddo = new DispatchDataOperation();
            DataTable dt = new DataTable();
            dt = ddo.GraphWareHouseInvoice(selectedText1, selectedText2, selectedText3, frmdt, todt);
            List<GraphData> chartData = new List<GraphData>();
            foreach (DataRow dr in dt.Rows)
            {
                chartData.Add(new GraphData
                {
                    w = Convert.ToString(dr["WareHouseName"]).Remove(10),
                    x = (Convert.ToInt32(dr["total_invoice"]))
                });
            }
            return chartData;
        }
        #endregion
        #region GraphSupplierWiseDispatch
        [WebMethod]
        public static List<GraphData> GraphSDisp(string selectedText1, string selectedText2, string selectedText3, string frmdt, string todt)
        {
            DispatchDataOperation ddo = new DispatchDataOperation();
            DataTable dt = new DataTable();
            dt = ddo.GraphSupplierDispatch(selectedText1, selectedText2, selectedText3, frmdt, todt);
            List<GraphData> chartData = new List<GraphData>();
            foreach (DataRow dr in dt.Rows)
            {
                chartData.Add(new GraphData
                {
                    w = Convert.ToString(dr["SupplierName"]).Remove(12),
                    x = (Convert.ToInt32(dr["No_of_dispatch"]))
                });
            }
            return chartData;
        ...
        protected void dropdownFYear_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (Session["lastSelectedCountryIndex"] != null)
            {
                ...
                if (dropdownFYear.SelectedIndex != -1)
                {
                    ...
                    List<GraphData> obj, obj1, obj2, obj3 = new List<GraphData>();
                    obj = GraphWDA(selectedText1, selectedText2, selectedText3, frmdt, todt);
                    obj1 = GraphWDisp(selectedText1, selectedText2, selectedText3, frmdt, todt);
                    obj2 = GraphWInvo(selectedText1, selectedText2, selectedText3, frmdt, todt);
                    obj3 = GraphSDisp(selectedText1, selectedText2, selectedText3, frmdt, todt);
